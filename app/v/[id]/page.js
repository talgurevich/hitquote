...
function PublicQuoteView() {
  ...
  const token = params?.id?.toString();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // First get settings and try to find proposal by token, if that fails try by ID
        const [{ data: st }] = await Promise.all([
          supabase.from('settings').select('*').limit(1).maybeSingle()
        ]);

        // Try to find proposal by share_token first, with fallback to ID
        let p = null;
        try {
          const { data: pByToken, error } = await supabase.from('proposal')
            .select('id, proposal_number, created_at, subtotal, discount_value, vat_rate, vat_amount, total, payment_terms, notes, include_discount_row, customer:customer_id(name, phone, email, address)')
          ...
          console.log('share_token column not found, trying by ID');
        }

        // If not found by token or token column doesn't exist, try by ID
        if (!p) {
          const { data: pById } = await supabase.from('proposal')
            .select('id, proposal_number, created_at, subtotal, discount_value, vat_rate, vat_amount, total, payment_terms, notes, include_discount_row, customer:customer_id(name, phone, email, address)')
            .eq('id', token)
            .maybeSingle();
          p = pById;
        ...
        if (!p) {
          setError('הצעה לא נמצאה או שהקישור אינו תקף');
          return;
        }

        // Then get proposal items using the proposal ID
        const { data: it } = await supabase.from('proposal_item')
          .select('id, custom_name, product_name, qty, unit_price, line_total, notes')
          .eq('proposal_id', p.id)
